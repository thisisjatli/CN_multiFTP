...
def downloadFile(clientSocket, downloadfileName, savefileName="newDownloadTestFile.pptx"):
    savefileName = "new" + downloadfileName[0].upper() + downloadfileName[1:]
    clientSocket.send(downloadfileName.encode())     # tell the server it's ready
    
    print("Start downloading...")
    try:
        filesize = int(clientSocket.recv(CHUNKSIZE).decode())
    except:
        mes = clientSocket.recv(CHUNKSIZE).decode()
        print("The server says:", mes)
        return
